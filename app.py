    ...
    if available_search_groups:
        ...
        if not model_comparison_df.empty:
            ...
            if hedonic_model:
                ...
                tooltips = {
                    "Dep. Variable": "Что это: Зависимая переменная (то, что мы моделируем). Обычно — логарифм цены автомобиля.",
                    "Model": "Что это: Тип используемой модели. OLS — обычный метод наименьших квадратов.",
                    "Method": "Что это: Метод оценки параметров модели. Обычно — Least Squares (наименьших квадратов).",
                    "Date": "Что это: Дата построения модели.",
                    "Time": "Что это: Время построения модели.",
                    "Sample": "Что это: Диапазон наблюдений, использованных в модели.",
                    "No. Observations": "Что это: Количество наблюдений (автомобилей), использованных для построения модели.",
                    "Df Residuals": "Что это: Степени свободы остатков (наблюдения минус количество параметров). Технический параметр.",
                    "Df Model": "Что это: Количество факторов в модели (без учета константы).",
                    "Covariance Type": "Что это: Тип используемой ковариационной матрицы ошибок. Обычно — стандартная оценка.",
                    ...
                    "Intercept": "Что это: Базовое значение логарифма цены, когда все остальные факторы равны нулю. Обычно не имеет прямого экономического смысла.",
                    "age": "Что это: Фактор 'возраст автомобиля'. Коэффициент показывает, на сколько в среднем изменяется логарифм цены с каждым дополнительным годом возраста.",
                    "mileage_km": "Что это: Фактор 'пробег автомобиля'. Коэффициент показывает, на сколько в среднем изменяется логарифм цены с каждым дополнительным километром пробега.",
                    "I(mileage_km ** 2)": "Что это: Квадрат пробега. Этот член добавлен, чтобы учесть нелинейную зависимость цены от пробега.",
                    "market_polovni_automobili": "Что это: Фактор 'рынок'. Показывает, на сколько логарифм цены на этом рынке отличается от базового рынка, при условии одинакового возраста и пробега.",
                    "coef": "Что это: Значение коэффициента. Показывает силу и направление влияния фактора. Отрицательный — уменьшает цену, положительный — увеличивает.",
                    "std err": "Что это: Стандартная ошибка. Показывает точность оценки коэффициента. Если ошибка велика по сравнению с коэффициентом, к его значению стоит относиться с осторожностью.",
                    "t": "Что это: t-статистика. Отношение коэффициента к его стандартной ошибке. Используется для расчета p-значения.",
                    "P>|t|": "Что это: p-значение. Если < 0.05, влияние фактора статистически значимо.",
                    "[0.025": "Что это: Начало 95% доверительного интервала. Истинное значение коэффициента с вероятностью 95% находится внутри этого интервала.",
                    "0.975]": "Что это: Конец 95% доверительного интервала. Истинное значение коэффициента с вероятностью 95% находится внутри этого интервала.",
                    ...
                    "Durbin-Watson": "Что это: Тест на автокорреляцию остатков. Значения от 1.5 до 2.5 обычно считаются приемлемыми.",
                    "Jarque-Bera (JB)": "Что это: Другой тест на нормальность остатков.",
                    "Prob(JB)": "Что это: Вероятность теста Jarque-Bera.",
                    "Cond. No.": "Что это: Число обусловленности. Предупреждает о потенциальной нестабильности модели. В данном случае высокое значение ожидаемо из-за связи 'пробега' и 'пробега в квадрате' и не является критической проблемой.",
                    "market_": "Что это: Бинарные индикаторы рынков; коэффициент — премия/дисконт к базовому рынку."
                }
